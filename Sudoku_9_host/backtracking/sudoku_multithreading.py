...
thread_ls = []
def solve_sudoku(arr, executors):
    global thread_ls
    ...
    for i in range(1, 10):
        if is_location_safe(arr, *location, i):
            ...
            thread_ls.append(executors.submit(solve_sudoku, new_arr, executors))
            if solve_sudoku(arr, executors):
                print(arr)
                return True
...
def solve(arr):
    ...
    solve_sudoku(arr, executors)
    rs = False
    for task in as_completed(thread_ls):
        rs = rs or task.result()
